                                        ...
                                        uint32_t tgt_peernum, TOX_GROUP_MOD_TYPE type)
{
    ...
    switch (type) {
        case TOX_MOD_KICK:
            eventstr = gettext("kicked");
            break;
        case TOX_MOD_BAN:
            eventstr = gettext("banned");
            break;
        case TOX_MOD_SILENCE:
            eventstr = gettext("silenced");
            break;
        default:
            return;
    }
    ...
    char timefrmt[TIME_STR_SIZE];
    get_time_str(timefrmt, sizeof(timefrmt));
    line_info_add(self, timefrmt, NULL, NULL, SYS_MSG, 1, colour, gettext("-!- %s has been %s by %s"), tgt_name, eventstr, src_name);
}
