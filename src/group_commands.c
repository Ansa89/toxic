...
void cmd_ignore(WINDOW *window, ToxWindow *self, Tox *m, int argc, char (*argv)[MAX_STR_SIZE])
{
    ...
    if (tox_group_toggle_ignore(m, self->num, peernum, 1) == -1) {
        line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Failed to ignore %s", nick));
        return;
    }
...
void cmd_unban(WINDOW *window, ToxWindow *self, Tox *m, int argc, char (*argv)[MAX_STR_SIZE])
{
    ...
    switch (ret) {
        ...
        case -2: {
            line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("You do not have permission to unban peers."));
            return;
        }
        default: {
...
void cmd_set_passwd(WINDOW *window, ToxWindow *self, Tox *m, int argc, char (*argv)[MAX_STR_SIZE])
{
    ...
    switch (ret) {
        case 0: {
            if (len > 0)
                line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Password has been set to %s"), passwd);
            else
                line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Password has been unset"));
            return;
        }
        case -2: {
            line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("You do not have permission to set the password"));
            return;
        }
        default: {
            line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Error setting password"));
            return;
        }
    }
}
...
void cmd_set_peerlimit(WINDOW *window, ToxWindow *self, Tox *m, int argc, char (*argv)[MAX_STR_SIZE])
{
    ...
    if (argc < 1) {
        ...
        if (maxpeers == -1) {
            line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Failed to retrieve peer limit"));
            return;
        }

        line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Peer limit is set to %d"), maxpeers);
        return;
    }
    ...
    if (maxpeers <= 0) {
        line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Peer limit must be a value greater than 0"));
        return;
    }
    ...
    switch (ret) {
        case 0: {
            line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Peer limit has been set to %d"), maxpeers);
            return;
        }
        case -2: {
            line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("You do not have permission to set the peer limit."));
            return;
        }
        default: {
            line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Failed to set the peer limit"));
            return;
        }
    }
}
...
void cmd_set_topic(WINDOW *window, ToxWindow *self, Tox *m, int argc, char (*argv)[MAX_STR_SIZE])
{
    ...
    line_info_add(self, timefrmt, NULL, NULL, SYS_MSG, 1, MAGENTA, gettext("-!- You set the topic to: %s"), topic);

    char tmp_event[MAX_STR_SIZE];
    snprintf(tmp_event, sizeof(tmp_event), gettext("set topic to %s"), topic);
...
{
    if (argc < 1) {
        line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Who do you want to unignore?"));
        return;
    }
    ...
    if (tox_group_toggle_ignore(m, self->num, peernum, 0) == -1) {
        line_info_add(self, NULL, NULL, NULL, SYS_MSG, 0, 0, gettext("Failed to unignore %s"), nick);
        return;
    }
    ...
    line_info_add(self, timefrmt, NULL, NULL, SYS_MSG, 1, BLUE, gettext("-!- You are no longer ignoring %s"), nick);
}
